...
class IrssiFormatter < DailyFileFormatter
  ...
  def dump_message_line(message, message_line, output_stream)
    date_str = Time.at(message['date']).strftime('%H:%M ')
    ...
    line = case message['event'].downcase
      ...
      when 'service'
        ...
        case message['action']['type'].downcase
          when 'chat_add_user'
            "*!* #{target_ref} [tg@#{target_ref}.users.telegram] has joined"
          when 'chat_add_user_link'
            "*!* #{user_ref} has joined [tg@#{user_ref}.users.telegram]"
          when 'chat_del_user'
            return if target['print_name'].to_s == ''
            if target_ref == user_ref
              "*!* #{user_ref} [tg@#{user_ref}.users.telegram] has quit"
            else
              "*!* #{target_ref} was kicked by #{user_ref} [Removed from group]"
            end
          when 'chat_rename'
            "*!* #{user_ref} changed the topic to '#{message['action']['title']}'"
          else nil
        end
      else nil
    end
